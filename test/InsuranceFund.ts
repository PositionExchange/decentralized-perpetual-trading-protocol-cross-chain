...
describe('Insurance Fund', async function () {
    ...
    describe("test deposit", async () => {
        it("given bonusBalance > 0 && depositAmount < bonusBalance, should take fund from bonus balance", async () => {
            ...
            // Set Trader bonus balance in InsuranceFund
            await insuranceFund.connect(deployer).setBonusBalance(positionManager.getAddress(), trader.getAddress(), BigNumber.from('0'))
            ...
            // const traderBonusBalanceInInsuranceFund = await insuranceFund.busdBonusBalances(positionManager.getAddress(), trader.getAddress())
        ...
        it("given bonusBalance in wallet > 0 && given bonusBalance in InsuranceFund > 0 && depositAmount + fee > bonusBalance, should take all fund from bonus balance", async () => {
            ...
            // Set Trader bonus balance in InsuranceFund
            await insuranceFund.connect(deployer).setBonusBalance(positionManager.getAddress(), trader.getAddress(), BigNumber.from('20'))
    ...
    describe("test withdraw, when partial close", async () => {
        ...
        it("given bonusBalance = 60 && withdrawAmount = 1070, expect user receive busd = 1010 and bonus = 60", async () => {
            ...
            await insuranceFund.connect(deployer).setBonusBalance(positionManager.getAddress(), trader.getAddress(), BigNumber.from('60'))
        ...
        it("given bonusBalance = 90 && withdrawAmount = 50, expect user receive busd = 0 and bonus = 50", async () => {
            ...
            await insuranceFund.connect(deployer).setBonusBalance(positionManager.getAddress(), trader.getAddress(), BigNumber.from('90'))
