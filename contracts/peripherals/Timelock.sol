...
contract Timelock is ITimelock {
    ...
    function initRewardRouter() external onlyAdmin {
        ...
        IHandlerTarget(_rewardRouter.feePlpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(_rewardRouter.stakedPlpTracker()).setHandler(rewardRouter, true);
    ...
    function setTokenConfig(
        ...
        IVault vault = IVault(_vault);
        require(vault.isWhitelistedTokens(_token), "Timelock: token not yet whitelisted");
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        ILpManager(glpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }
