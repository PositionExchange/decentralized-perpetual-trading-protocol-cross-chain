...
contract RewardRouter is IRewardRouter, ReentrancyGuard, Governable, Initializable{
  ...
  function batchStakePosiForAccount(address[] memory _accounts, uint256[] memory _amounts) external nonReentrant onlyGov {
      address _posi = posi;
  ...
  function stakePosiForAccount(address _account, uint256 _amount) external nonReentrant onlyGov {
  ...
  function mintAndStakePlp(address _token, uint256 _amount, uint256 _minUsdp, uint256 _minPlp) external nonReentrant returns (uint256) {
      require(_amount > 0, "RewardRouter: invalid _amount");

      address account = msg.sender;
      uint256 plpAmount = ILpManager(plpManager).addLiquidityForAccount(account, account, _token, _amount, _minUsdp, _minPlp);
  ...
  function mintAndStakePlpETH(uint256 _minUsdp, uint256 _minPlp) external payable nonReentrant returns (uint256) {
      ...
      address account = msg.sender;
      uint256 plpAmount = ILpManager(plpManager).addLiquidityForAccount(address(this), account, weth, msg.value, _minUsdp, _minPlp);
  ...
  function unstakeAndRedeemPlp(address _tokenOut, uint256 _plpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
      ...
      uint256 amountOut = ILpManager(plpManager).removeLiquidityForAccount(account, _tokenOut, _plpAmount, _minOut, _receiver);
  ...
  function unstakeAndRedeemPlpETH(uint256 _plpAmount, uint256 _minOut, address payable _receiver) external nonReentrant returns (uint256) {
      ...
      IRewardTracker(stakedPlpTracker).unstakeForAccount(account, feePlpTracker, _plpAmount, account);
      IRewardTracker(feePlpTracker).unstakeForAccount(account, plp, _plpAmount, account);
      uint256 amountOut = ILpManager(plpManager).removeLiquidityForAccount(account, weth, _plpAmount, _minOut, address(this));
  ...
  function handleRewards(
      ...
      if (_shouldClaimPosi) {
          uint256 posiAmount0 = IVester(posiVester).claimForAccount(account, account);
          uint256 posiAmount1 = IVester(plpVester).claimForAccount(account, account);
      ...
      if (_shouldClaimEsPosi) {
          uint256 esPosiAmount0 = IRewardTracker(stakedPosiTracker).claimForAccount(account, account);
          uint256 esPosiAmount1 = IRewardTracker(stakedPlpTracker).claimForAccount(account, account);
      ...
      if (_shouldStakeMultiplierPoints) {
          uint256 bnPosiAmount = IRewardTracker(bonusPosiTracker).claimForAccount(account, account);
          if (bnPosiAmount > 0) {
              IRewardTracker(feePosiTracker).stakeForAccount(account, account, bnPosi, bnPosiAmount);
          }
      }

      if (_shouldClaimWeth) {
          if (_shouldConvertWethToEth) {
              uint256 weth0 = IRewardTracker(feePosiTracker).claimForAccount(account, address(this));
  ...
  function signalTransfer(address _receiver) external nonReentrant {
      require(IERC20(posiVester).balanceOf(msg.sender) == 0, "RewardRouter: sender has vested tokens");
  ...
  function acceptTransfer(address _sender) external nonReentrant {
      require(IERC20(posiVester).balanceOf(_sender) == 0, "RewardRouter: sender has vested tokens");
      ...
      uint256 stakedPosi = IRewardTracker(stakedPosiTracker).depositBalances(_sender, posi);
      if (stakedPosi > 0) {
          _unstakePosi(_sender, posi, stakedPosi, false);
          _stakePosi(_sender, receiver, posi, stakedPosi);
      }

      uint256 stakedEsPosi = IRewardTracker(stakedPosiTracker).depositBalances(_sender, esPosi);
      if (stakedEsPosi > 0) {
          _unstakePosi(_sender, esPosi, stakedEsPosi, false);
          _stakePosi(_sender, receiver, esPosi, stakedEsPosi);
      }

      uint256 stakedBnPosi = IRewardTracker(feePosiTracker).depositBalances(_sender, bnPosi);
      if (stakedBnPosi > 0) {
          IRewardTracker(feePosiTracker).unstakeForAccount(_sender, bnPosi, stakedBnPosi, _sender);
          IRewardTracker(feePosiTracker).stakeForAccount(_sender, receiver, bnPosi, stakedBnPosi);
      }
  ...
  function _validateReceiver(address _receiver) private view {
      require(IRewardTracker(stakedPosiTracker).averageStakedAmounts(_receiver) == 0, "RewardRouter: stakedPosiTracker.averageStakedAmounts > 0");
      require(IRewardTracker(stakedPosiTracker).cumulativeRewards(_receiver) == 0, "RewardRouter: stakedPosiTracker.cumulativeRewards > 0");

      require(IRewardTracker(bonusPosiTracker).averageStakedAmounts(_receiver) == 0, "RewardRouter: bonusPosiTracker.averageStakedAmounts > 0");
      require(IRewardTracker(bonusPosiTracker).cumulativeRewards(_receiver) == 0, "RewardRouter: bonusPosiTracker.cumulativeRewards > 0");

      require(IRewardTracker(feePosiTracker).averageStakedAmounts(_receiver) == 0, "RewardRouter: feePosiTracker.averageStakedAmounts > 0");
      require(IRewardTracker(feePosiTracker).cumulativeRewards(_receiver) == 0, "RewardRouter: feePosiTracker.cumulativeRewards > 0");

      require(IVester(posiVester).transferredAverageStakedAmounts(_receiver) == 0, "RewardRouter: posiVester.transferredAverageStakedAmounts > 0");
      require(IVester(posiVester).transferredCumulativeRewards(_receiver) == 0, "RewardRouter: posiVester.transferredCumulativeRewards > 0");
      ...
      require(IVester(plpVester).transferredAverageStakedAmounts(_receiver) == 0, "RewardRouter: posiVester.transferredAverageStakedAmounts > 0");
      require(IVester(plpVester).transferredCumulativeRewards(_receiver) == 0, "RewardRouter: posiVester.transferredCumulativeRewards > 0");

      require(IERC20(posiVester).balanceOf(_receiver) == 0, "RewardRouter: posiVester.balance > 0");
  ...
  function _compoundPosi(address _account) private {
      uint256 esPosiAmount = IRewardTracker(stakedPosiTracker).claimForAccount(_account, _account);
      if (esPosiAmount > 0) {
          _stakePosi(_account, _account, esPosi, esPosiAmount);
      }

      uint256 bnPosiAmount = IRewardTracker(bonusPosiTracker).claimForAccount(_account, _account);
      if (bnPosiAmount > 0) {
          IRewardTracker(feePosiTracker).stakeForAccount(_account, _account, bnPosi, bnPosiAmount);
      }
  }

  function _compoundPlp(address _account) private {
      uint256 esPosiAmount = IRewardTracker(stakedPlpTracker).claimForAccount(_account, _account);
      if (esPosiAmount > 0) {
          _stakePosi(_account, _account, esPosi, esPosiAmount);
      }
  }

  function _stakePosi(address _fundingAccount, address _account, address _token, uint256 _amount) private {
      require(_amount > 0, "RewardRouter: invalid _amount");

      IRewardTracker(stakedPosiTracker).stakeForAccount(_fundingAccount, _account, _token, _amount);
      IRewardTracker(bonusPosiTracker).stakeForAccount(_account, _account, stakedPosiTracker, _amount);
      IRewardTracker(feePosiTracker).stakeForAccount(_account, _account, bonusPosiTracker, _amount);

      emit StakePosi(_account, _token, _amount);
  }

  function _unstakePosi(address _account, address _token, uint256 _amount, bool _shouldReduceBnPosi) private {
      require(_amount > 0, "RewardRouter: invalid _amount");

      uint256 balance = IRewardTracker(stakedPosiTracker).stakedAmounts(_account);

      IRewardTracker(feePosiTracker).unstakeForAccount(_account, bonusPosiTracker, _amount, _account);
      IRewardTracker(bonusPosiTracker).unstakeForAccount(_account, stakedPosiTracker, _amount, _account);
      IRewardTracker(stakedPosiTracker).unstakeForAccount(_account, _token, _amount, _account);
      ...
      if (_shouldReduceBnPosi) {
          ...
          uint256 stakedBnPosi = IRewardTracker(feePosiTracker).depositBalances(_account, bnPosi);
          if (stakedBnPosi > 0) {
              uint256 reductionAmount = stakedBnPosi.mul(_amount).div(balance);
              IRewardTracker(feePosiTracker).unstakeForAccount(_account, bnPosi, reductionAmount, _account);
